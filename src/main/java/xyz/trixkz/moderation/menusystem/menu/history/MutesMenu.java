...
public class MutesMenu extends PaginatedMenu {
    ...
    public MutesMenu(PlayerMenuUtil playerMenuUtil, OfflinePlayer offlinePlayer) {
    ...
    public String getMenuName() {
        return Utils.translate("&eLook at " + this.offlinePlayer.getName() + "'s mutes...");
    }
    ...
    public void handleMenu(InventoryClickEvent event) {
        ...
        if (event.getView().getTitle().equalsIgnoreCase(Utils.translate("&eLook at " + this.offlinePlayer.getName() + "'s mutes..."))) {
    ...
    public void setMenuItems(Player player) {
        ...
        for (int i = 0; i < this.getMaxItemsPerPage(); i++) {
            ...
            if (this.punishments.get(i) != null) {
                ...
                ItemStackButton mute = new ItemStackButton(
                        ...
                        new String[]{
                                ...
                                "&fDuration: &d" + (punishment.getType() == PunishmentType.MUTE ? (punishment.isExpired() ? "None" : "Permanent") : (punishment.getRemainingDurationAsALong() == 0L ? "None" : Utils.makeTimeReadable(punishment.getRemainingDurationAsALong()))),
                                ...
                                "",
                                (punishment.isUnmuted() ? "&fUnmuted by: &d" + (punishment.isUnmutedByConsole() ? "Console" : this.main.getServer().getOfflinePlayer(punishment.getUnmutedBy()).getName()) : ""),
                                (punishment.isUnmuted() ? "&fUnmuted at: &d" + Utils.getAddedAtDate(punishment.getUnmutedAt()) : ""),
                                (punishment.isUnmuted() ? "&fUnmuted reason: &d" + punishment.getUnmutedReason() : "")
                        },
                ...
                this.inventory.addItem(mute.makeItemStack());
            }
        }
    }
}
